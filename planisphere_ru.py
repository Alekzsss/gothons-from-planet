from planisphere import Room, load_room, name_room
from random import randint

# second map
quips_ru = [
        "Ты умер. Ты не умеешь играть.",
        "Твоя мама гордилась бы тобой...если бы она была умне.",
        "Ты неудачник.",
        "У меня есть маленький щенок, который сыграет лучше тебя.",
        "Ты хуже, чем шутки твоего отца."
    ]

START = 'central_corridor_ru'

rooms = {

    'central_corridor_ru': Room("Центральный коридор",
                             """
    ...
    'laser_weapon_armory_ru': Room("Оружейная лаборатория",
                                """
    ...
    'the_bridge_ru': Room("Топливный отсек",
                       """
    ...
    'escape_pod_ru': Room("Спасательная капсула",
                       """
    ...
    'generic_death_ru': Room("death", quips_ru[randint(0, len(quips_ru) - 1)])
}

rooms['escape_pod_ru'].add_paths({
    '2': rooms.get('the_end_winner_ru'),
    '*': rooms.get('the_end_loser_ru')
})

rooms['the_bridge_ru'].add_paths({
    'throw the bomb': rooms.get('generic_death_ru'),
    'slowly place the bomb': rooms.get('escape_pod_ru')
})

rooms['laser_weapon_armory_ru'].add_paths({
    '0132': rooms.get('the_bridge_ru'),
    '*': rooms.get('generic_death_ru')
})

rooms['central_corridor_ru'].add_paths({
    'shoot!': rooms.get('generic_death_ru'),
    'dodge!': rooms.get('generic_death_ru'),
    'tell a joke': rooms.get('laser_weapon_armory_ru')
})
